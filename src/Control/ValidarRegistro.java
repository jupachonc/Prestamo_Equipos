...
public class ValidarRegistro {
    ...
    public String verificarRegistroAdmin(String nombre, String apellido, int documento, String email, String contrasena, String reContrasena) {
        Pattern VALID_EMAIL_ADDRESS_REGEX = Pattern.compile("^[A-Z0-9._%+-]+@[A-Z0-9.-]+\\.[A-Z]{2,6}$", Pattern.CASE_INSENSITIVE);
        Matcher matcher = VALID_EMAIL_ADDRESS_REGEX.matcher(email);
        Pattern VALID_DOCUMENT_REGEX = Pattern.compile("^[0-9]{7,10}$");
        Matcher matcher2 = VALID_DOCUMENT_REGEX.matcher(String.valueOf(documento));
        ...
        if (!verificarLongitud(nombre, 64)) {
            ...
            usuario = new Usuario(nombre, apellido, documento, email, contrasena, 1);
            dao.crear(usuario);
            return ("Usuario registrado");
